...
export async function deployFactory(hre: HardhatRuntimeEnvironment) {
  // Private key of the account used to deploy
  const wallet = getWallet();
  const deployer = new Deployer(hre, wallet);
  const factoryArtifact = await deployer.loadArtifact("AAFactory");
  const aaArtifact = await deployer.loadArtifact("SellableWallet");
...
export async function deployAccount(hre: HardhatRuntimeEnvironment, factoryAddress: string, currentSignerAddress: string) {
  const wallet = getWallet();
  ...
  const accountAddress = await generateAccountAddress(aaFactory, currentSignerAddress);

  const accountArtifact = await hre.artifacts.readArtifact("SellableWallet");
  
  console.log(`Sellable wallet account deployed on address ${accountAddress}`);
  return new Contract(accountAddress, accountArtifact.abi, wallet);
}

export async function generateAccountAddress(factory: Contract, currentSignerAddress: string) {
  const abiCoder = new ethers.utils.AbiCoder();
  const accountAddress = utils.create2Address(
      factory.address,
      await factory.aaBytecodeHash(),
      getSalt(),
      abiCoder.encode(["address"], [currentSignerAddress]),
  );
...
//  return "0x" + hexlify(ethers.utils.toUtf8Bytes("Not today")).slice(2).padStart(64, "0");
}